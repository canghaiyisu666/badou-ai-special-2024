...
def log(text, array=None):
    """
    打印文本消息。如果提供了 Numpy 数组，则打印数组的形状、最小值和最大值。

    参数:
    text (str): 要打印的文本消息。
    array (numpy.ndarray, 可选): 要打印信息的 Numpy 数组，如果提供的话。
    """
    # 检查是否提供了数组，以增强输出消息
...
if __name__ == "__main__":
    ...
    for name in loss_names:
        ...
        if layer.output in model.losses:
            continue
        loss = (
            tf.reduce_mean(layer.output, keepdims=True) * config.LOSS_WEIGHTS.get(name, 1.))
        model.add_loss(loss)
